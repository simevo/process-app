...
function init() {
  ...
  if (window.openDatabase) {
    var db = openDatabase(current_case + '/persistency.db', '1.0', 'persistency database', 2 * 1024 * 1024);
  ...
  ko.applyBindings(viewModelConfigure, configure);
...
function updateServers(update) {
  ...
  viewModel.servers.servers().sort(function(left, right) {
	...
	var uuid = a1.uuid();
	background.style.backgroundImage = 'url('+uuid+'/background-landing.jpg)';
}// updateServers
...
function change_node(targetid) {
  xmlhttp.open("GET", "children.json", false);
  xmlhttp.send();
  children = JSON.parse(xmlhttp.responseText);
  ko.mapping.fromJS(children, mapping, viewModelChildren);

  xmlhttp.open("GET", "inputs.json", false);
  xmlhttp.send();
  inputs = JSON.parse(xmlhttp.responseText);
  ko.mapping.fromJS(inputs, mapping, viewModelInputs);

  xmlhttp.open("GET", "outputs.json", false);
  xmlhttp.send();
  outputs = JSON.parse(xmlhttp.responseText);
